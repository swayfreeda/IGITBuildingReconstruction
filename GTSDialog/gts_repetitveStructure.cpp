...
//---------------------------------------------EMTraining-----------------------------------------------//
// EM is applied to the get the initial labels
// indeed Kmeans is used and the the results is better than EM
...
//---------------------------------------EMPredict---------------------------------------------------//
// assign a background or foreground label to each pixel through the GMM models obtained from
// EMTraining
...
//---------------------------------------transfromedPoint---------------------------------------------//
// apply a homography transformation to each point
...
GTSDetectionDialog::GTSDetectionDialog(QWidget *parent)
{
    ...
    m_scale_ = 1.00;
    m_without_rectification_ = false;
...
QVector<QVector<double> > GTSDetectionDialog::getInitialLabels()
{
    ...
    QImage img = m_rected_image_.copy(0, 0, m_image_.width(), m_image_.height());
    m_rected_image_.convertToFormat(QImage::Format_RGB32);
    ...
    foreach(QPoint pt, m_bg_samples)
    {
        ...
        sample[0] = (float)imgBits[lineNum + j*4 + 0];
        sample[1] = (float)imgBits[lineNum + j*4 + 1];
        sample[2] = (float)imgBits[lineNum + j*4 + 2];
...
#if 0
    QVector<QVector<double> > initial_labels;
    initial_labels.resize(m_img_h_);

    QString filename("Y.txt");
    ...
    while(!in.atEnd())
    {
        ...
        for(int i=0; i< m_img_w_; i++)
        {
            initial_labels[line_num].append(fields.takeFirst().toDouble());
        }
        line_num ++;
    }
...
void GTSDetectionDialog::rectification()
{
...
#endif
    m_H_ = cv::findHomography(src_pts, dst_pts, cv::RANSAC, 2);
    ...
    uchar *imgBits0 = m_image_.bits();

    int height = m_image_.height();
    int width = m_image_.width();

    QImage img(QSize(width, height), QImage::Format_RGB32);
    m_rected_image_ = img.copy(0, 0, width, height);
    uchar *imgBits1 = m_rected_image_.bits();
    ...
    for(int i=0; i< height; i++)
    {
        ...
        for(int j=0; j< width; j++)
        {
            ...
            {
                imgBits1[lineNum + j*4 + 0] = imgBits0[ii* width *4 + jj* 4 + 0 ];
                imgBits1[lineNum + j*4 + 1] = imgBits0[ii* width *4 + jj* 4 + 1 ];
                imgBits1[lineNum + j*4 + 2] = imgBits0[ii* width *4 + jj* 4 + 2 ];
            }
            else{
                imgBits1[lineNum + j*4 + 0] = 0;
                imgBits1[lineNum + j*4 + 1] = 0;
                imgBits1[lineNum + j*4 + 2] = 0;
            }
        }
    }
   ...
   // m_GTS_pos_.clear();
    ...
    for(int i=0; i< row_indices.size(); i++)
    {
        for(int j=0; j< col_indices.size(); j++)
        {
            ...
            QVector<QPoint> quad;
            quad.append(topLeft);
            quad.append(topRight);
            quad.append(bottomRight);
            quad.append(bottomLeft);
            m_detected_gts_retected_.append(quad);

            if(m_without_rectification_ == false)
            {
                //转化都原来的空间
                QVector<QPoint> quad;
                quad.append( transformedPoint(topLeft, m_H_.inv()));
                quad.append( transformedPoint(topRight, m_H_.inv()));
                quad.append( transformedPoint(bottomRight, m_H_.inv()));
                quad.append( transformedPoint(bottomLeft, m_H_.inv()));

                // scale
                for(int i=0; i< quad.size(); i++)
                {
                    quad[i].setX(quad[i].x()/m_scale_);
                    quad[i].setY(quad[i].y()/m_scale_);
                }

                m_detected_gts_.append(quad);
            }
            if( m_without_rectification_ == true)
            {
                foreach(QVector<QPoint> quad, m_detected_gts_retected_)
                {
                    // scale
                    for(int i=0; i< quad.size(); i++)
                    {
